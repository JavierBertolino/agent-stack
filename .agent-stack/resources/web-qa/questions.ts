...
//
// Business checks are project-agnostic: they verify explicit business
...
export function buildQuestions(dims: Dim[] = DIMENSIONS): Record<string, QuestionDef> {
  ...
  if (want("business")) {
    q.business_rule_respected = {
      type: "noul",
      instructions:
        "Does the observed behavior comply with the explicit business constraints in governance and the expected result for `test_goal`?",
    };
    q.forbidden_side_effects_avoided = {
      type: "noul",
      instructions:
        "Does `trace` avoid side effects outside the allowed scope for `test_goal`?",
    };
    q.state_transition_valid = {
      type: "noul",
      instructions:
        "Does the observed state transition follow the allowed workflow and invariants for `test_goal`?",
    };
    q.traceability_present = {
      type: "noul",
      instructions:
        "For sensitive or auditable actions, does `trace` preserve the actor, action, timestamp, reason, and relevant object identifiers required by the project's governance?",
      criteria: {
        true: "Actor, action, timestamp, reason, and object identifiers are present in the trace.",
        false: "Required audit fields are missing from the trace.",
      },
    };
  }
  ...
  q.domain = {
    ...
    criteria: {
      ...
      business_logic: "Business rules, side effects, state transitions, traceability.",
    },
  };
  q.severity = {
    type: "score",
