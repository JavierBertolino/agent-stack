...
# by default; the wizard offers every combination (multi-select).
...
# Jev QA provider routing (project-level; credentials stay user-level).
# none | typesafe | vercel | gateway. OpenRouter is preview-only.
JEV_PROVIDER=none
JEV_MODEL=jev-latest
JEV_GATEWAY_URL=
JEV_CONFIGURED_NOW=0
JEV_CONFIGURED_LATER=0
...
Commands:
  ...
  auth jev [--validate-only] [--provider=typesafe|vercel|gateway]
                               Configure the Jev QA provider (user-level, never in repo).

  astack is the canonical command: astack init, astack sync, astack check,
  astack doctor, astack auth jev, astack update, astack upgrade, ...
...
jev_env_file() {
  printf '%s\n' "${XDG_CONFIG_HOME:-$HOME/.config}/astack/env"
}

jev_legacy_env_file() {
  printf '%s\n' "${XDG_CONFIG_HOME:-$HOME/.config}/agent-stack/env"
}

jev_value_from_env_file() {
  key=$1
  for candidate in "$(jev_env_file)" "$(jev_legacy_env_file)"; do
...
jev_is_configured() {
  ...
  [ -n "${JEV_GATEWAY_API_KEY:-}" ] && return 0
  jev_value_from_env_file TYPESAFE_API_KEY >/dev/null 2>&1 && return 0
  jev_value_from_env_file AI_GATEWAY_API_KEY >/dev/null 2>&1 && return 0
  jev_value_from_env_file JEV_GATEWAY_API_KEY >/dev/null 2>&1 && return 0
  return 1
}
...
resolve_jev() {
  ...
  JEV_MODEL=$(get_config JEV_MODEL jev-latest)
  JEV_GATEWAY_URL=$(get_config JEV_GATEWAY_URL '')

  if [ "$INTERACTIVE_WIZARD" = 1 ]; then
    configure_jev_interactive
  fi
}
...
configure_delivery_interactive() {
  ...
  if [ "$SPECS_MODE" = local ]; then
    SPECS_REPOSITORY=
    printf '\n%s\n' 'OpenSpec specs will remain in this code repository.'
    printf '%s\n' 'No external specs repository is required.'
  else
    printf '\n%s\n' 'Agent Stack will keep OpenSpec as the source of truth in this repository'
    printf '%s\n' 'and mirror finalized specs to another GitHub repository.'
    printf '\n'
    while :; do
      printf 'GitHub specs repository [%s]: ' "$SPECS_REPOSITORY"
    ...
    printf 'Base branch [%s]: ' "$SPECS_REPOSITORY_BASE_BRANCH"
...
configure_jev_interactive() {
  printf '\n%s\n' '◆ QA'
  printf '\n%s\n' 'Agent Stack can use TypeSafe Jev to evaluate web and mobile QA checkpoints.'
  printf '\n'

  if jev_is_configured; then
    printf '%s\n' '✓ Jev already configured'
    printf 'Reconfigure Jev? [y/N]: '
    IFS= read -r answer || die 'interactive configuration aborted'
    case "$answer" in
      y|Y|yes|YES) ;;
      *) return 0 ;;
    esac
  ...
  case "$JEV_MENU_CHOICE" in
    1) provider=typesafe ;;
    2) provider=vercel ;;
    3) provider=gateway ;;
  esac
  if auth_jev_provider "$provider"; then
    JEV_CONFIGURED_NOW=1
    JEV_CONFIGURED_LATER=0
  else
    JEV_CONFIGURED_LATER=1
  fi
...
configure_models_interactive() {
  wizard_step 'Configure Agent Stack for this project.'
  printf '\n%s\n' '◆ Models (Enter keeps the current value)'
...
persist_selection() {
  ...
  set_config JEV_PROVIDER "$JEV_PROVIDER"
  set_config JEV_MODEL "$JEV_MODEL"
  set_config JEV_GATEWAY_URL "$JEV_GATEWAY_URL"
}
...
AGENT_STACK_VERSION=0.1.0
MAX_CORRECTIVE_ROUNDS=3
TIMEBOX_MINUTES=25

# Specification publication policy.
...
# platform. All integrations are opt-in by default; the interactive wizard
# can enable any combination (Linear, Trello, Maestro, or none).
MCP_LINEAR_ENABLED=0
MCP_LINEAR_NAME=linear
MCP_LINEAR_URL=https://mcp.linear.app/mcp
MCP_TRELLO_ENABLED=0
...
# Jev QA provider routing (project-level; credentials stay user-level and
# are never stored here). Provider: none, typesafe, vercel, or gateway.
# OpenRouter is preview-only until its System One transport is verified.
JEV_PROVIDER=none
JEV_MODEL=jev-latest
JEV_GATEWAY_URL=
...
jev_store_env() {
  # jev_store_env KEY=VALUE... — append or replace entries in the user-level
  # env file. Secrets are never printed.
  env_file=$(jev_env_file)
  ...
  for assignment in "$@"; do
    ...
    awk -F= -v wanted="$key" -v replacement="$assignment" '
      BEGIN { found = 0 }
      $1 == wanted {
        if (!found) print replacement
        found = 1
  ...
  printf '✓ Stored outside the repository\n'
}
...
jev_prompt_secret() {
  ...
  if [ -n "$(eval "printf '%s' \"\${$env_name:-}\"")" ]; then
    eval "JEV_SECRET_VALUE=\"\${$env_name}\""
    return 0
  fi
  [ -t 0 ] || die "$env_name is not set; re-run interactively or export it first"
  printf '%s (input hidden): ' "$prompt_label"
  stty -echo 2>/dev/null || true
  IFS= read -r entered || true
  stty echo 2>/dev/null || true
  printf '\n'
  [ -n "$entered" ] || die 'no value entered'
  JEV_SECRET_VALUE=$entered
}

jev_validate_typesafe() {
  key=$1
  endpoint=${2:-https://api.typesafe.ai/v1/models}
  ...
  if command -v curl >/dev/null 2>&1; then
    status=$(curl -fsS -o /dev/null -w '%{http_code}' --max-time 15 \
      -H "Authorization: Bearer $key" "$endpoint" 2>/dev/null || true)
  else
    status=$(JEV_VALIDATE_KEY="$key" JEV_VALIDATE_URL="$endpoint" node -e "fetch(process.env.JEV_VALIDATE_URL,{headers:{Authorization:'Bearer '+process.env.JEV_VALIDATE_KEY}}).then(r=>console.log(r.status)).catch(()=>console.log('000'))" 2>/dev/null || true)
  fi
  case "$status" in
    200) return 0 ;;
    *) die "Jev credential validation failed (status ${status:-unknown})" ;;
  esac
}

auth_jev_provider() {
  # auth_jev_provider typesafe|vercel|gateway [--validate-only]
  provider=$1
  shift || true
  validate_only=0
  for a in "$@"; do
  ...
  case "$provider" in
    typesafe)
      ...
      jev_prompt_secret 'TypeSafe API key' TYPESAFE_API_KEY
      key=$JEV_SECRET_VALUE
      printf 'Validating credentials...\n'
      jev_validate_typesafe "$key" 'https://api.typesafe.ai/v1/models'
      printf '✓ Jev credentials are valid\n'
      [ "$validate_only" = 1 ] && return 0
      jev_store_env "TYPESAFE_API_KEY=$key" 'JEV_PROVIDER=typesafe'
      JEV_PROVIDER=typesafe
      ;;
    vercel)
      printf '\nVercel AI Gateway (Jev model typesafe-ai/jev-latest)\n'
      jev_prompt_secret 'Vercel AI Gateway API key' AI_GATEWAY_API_KEY
      key=$JEV_SECRET_VALUE
      printf 'Validating credentials...\n'
      jev_validate_typesafe "$key" "${JEV_GATEWAY_URL:-https://ai-gateway.vercel.sh}/v1/models"
      printf '✓ Jev credentials are valid\n'
      [ "$validate_only" = 1 ] && return 0
      jev_store_env "AI_GATEWAY_API_KEY=$key" 'JEV_PROVIDER=vercel' 'JEV_MODEL=typesafe-ai/jev-latest'
      JEV_PROVIDER=vercel
      JEV_MODEL=typesafe-ai/jev-latest
      ;;
    gateway)
      printf '\nCloudflare AI Gateway / compatible gateway\n'
      printf '%s\n' 'The endpoint must expose a compatible Jev/evaluation request contract.'
      gateway_url=${JEV_GATEWAY_URL:-}
      if [ -z "$gateway_url" ]; then
        [ -t 0 ] || die 'JEV_GATEWAY_URL is not set; re-run interactively or export it first'
        printf 'Gateway URL: '
        IFS= read -r gateway_url || true
        [ -n "$gateway_url" ] || die 'no URL entered'
      fi
      JEV_GATEWAY_URL=$gateway_url
      jev_prompt_secret 'Gateway API key' JEV_GATEWAY_API_KEY
      key=$JEV_SECRET_VALUE
      if [ -z "${JEV_MODEL:-}" ] || [ "$JEV_MODEL" = jev-latest ]; then
        if [ -t 0 ]; then
          printf 'Jev model [%s]: ' "${JEV_MODEL:-jev-latest}"
          IFS= read -r model_answer || true
          [ -n "$model_answer" ] && JEV_MODEL=$model_answer
        fi
        JEV_MODEL=${JEV_MODEL:-jev-latest}
      fi
      printf 'Validating credentials...\n'
      jev_validate_typesafe "$key" "$gateway_url"
      printf '✓ Jev credentials are valid\n'
      [ "$validate_only" = 1 ] && return 0
      jev_store_env "JEV_GATEWAY_API_KEY=$key" 'JEV_PROVIDER=gateway' "JEV_MODEL=$JEV_MODEL"
      JEV_PROVIDER=gateway
      JEV_GATEWAY_URL=$gateway_url
      ;;
    *)
      die "unknown Jev provider: $provider (use typesafe, vercel, or gateway)"
      ;;
  esac

  # Credentials stay user-level; provider routing is project-level and is
  # recorded only when this runs inside an initialized project.
  key=
  JEV_SECRET_VALUE=
  if [ -f "$CONFIG_FILE" ]; then
    PERSIST_SELECTION=1
...
auth_jev() {
  ...
  for a in "$@"; do
    case "$a" in
      --validate-only) validate_only=1 ;;
      --provider=*) provider_arg=${a#--provider=} ;;
      *) die "unknown auth option: $a (use --validate-only or --provider=typesafe|vercel|gateway)" ;;
    esac
  done

  if [ -z "$provider_arg" ]; then
    if [ ! -t 0 ]; then
      # Non-interactive default preserves the historical behavior: validate
      # the TypeSafe direct key from the environment.
      [ -n "${TYPESAFE_API_KEY:-}" ] || die 'TYPESAFE_API_KEY is not set'
      provider_arg=typesafe
    else
      printf '\n%s\n' 'Jev provider'
      printf '\n'
      printf '  1. TypeSafe direct\n'
      printf '  2. Vercel AI Gateway\n'
      printf '  3. Cloudflare / compatible gateway\n'
      printf '  4. OpenRouter (preview)\n'
      printf '\nSelect provider [1]: '
      IFS= read -r provider_choice || die 'interactive configuration aborted'
      case "${provider_choice:-1}" in
        1) provider_arg=typesafe ;;
        2) provider_arg=vercel ;;
        3) provider_arg=gateway ;;
        4)
          printf '\n%s\n' 'OpenRouter exposes Jev, but Agent Stack does not yet have a verified'
          printf '%s\n' 'System One-compatible transport for this route.'
          printf '\n%s\n' 'Choose TypeSafe direct, Vercel AI Gateway, or a compatible gateway.'
          exit 2
          ;;
        *) die "unknown provider choice: $provider_choice" ;;
      esac
    fi
  fi

  if [ "$validate_only" = 1 ]; then
    auth_jev_provider "$provider_arg" --validate-only
  else
    auth_jev_provider "$provider_arg"
  fi
}
...
record_kit_version() {
  ...
  [ -n "$version" ] || return 0
  ensure_dir "$ROOT/.agent-stack"
  printf '%s\n' "$version" > "$ROOT/.agent-stack/.kit-version"
}

print_jev_status() {
  if [ "$JEV_CONFIGURED_NOW" = 1 ] || jev_is_configured; then
    printf '\n%s\n' '✓ Jev configured'
    return 0
  fi
  printf '\n%s\n' 'Jev is not configured yet.'
  printf '\n%s\n' 'Configure it later with:'
  printf '\n%s\n' '  astack auth jev'
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --validate-only)
      ...
      ;;
    --provider=*)
      AUTH_ARGS="$AUTH_ARGS --provider=${1#--provider=}"
      ;;
    --root)
